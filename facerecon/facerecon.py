...
def argparser():
    ...
    ps_help = "Name of predecessor step corresponding to sub-directory in data/facelandmarks."
...
def read_data(im_path: str, lm_path: str, lm3d_std: NDArray, to_tensor: bool=True)->Tuple:
    """
...
def run(args: argparse.Namespace, logger: Logger):
    ...
    if args.benchmark in SUPPORTED_BENCHMARKS:
        ...
        try:
            ...
            logger.error(f"{im_src_dir} does not exist. Run the facerestore step to generate.")
            logger.error(e)
            sys.exit(1)
        try:
            lm_folder = DatasetFolder(lm_src_dir, _loader, extensions=("txt", ".txt"))
        except Exception as e:
            logger.error(f"{lm_src_dir} does not exist. Run the facelandmarks step to generate.")
            logger.error(e)
            sys.exit(1)
